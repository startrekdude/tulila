...
cookie = urandom(16).hex()
solution = urandom(16).hex()
set_solution(solution)

# Calculate Alice's x and gx
x = SystemRandom().randrange(p)
gx = pow(g, x, p)
send({"gx": gx}, "bob", network="internet")
...
while True:
	...
	# Retrieve a secret from Bob that Mallory can't see
	# This is used to force Mallory to do a key-exchange with both Bob and Alice
	if msg.network == "challenge_init":
		bobs_secret = bytes.fromhex(msg.data["secret"])
		continue
	
	if "gy" in msg.data and key is None:
		gy = msg.data["gy"]
		key = convert_shared_secret_to_AES_key(pow(gy, x, p))
		
		send(
			encrypt_message(key, "handshake_1_" + cookie),
			"bob",
			network="internet"
		)
	...
	if "ct" in msg.data and "iv" in msg.data:
		...
		if pt == "handshake_2_" + xor(bytes.fromhex(cookie), bobs_secret).hex():
			send(
				encrypt_message(key, solution),
				"bob",
				network="internet"
			)
